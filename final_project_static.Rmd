...
```{r, echo = TRUE, message=FALSE, warning=FALSE}
topic_percent_cite <- dplyr::summarise(group_by(papers, year, topic), sum_citaition = sum(n_citation))
...
ggplot(title_length_citation, aes(x=title_length, y=mean_citation)) +
  ...
  ylab("Citations") +
  xlab("Title Length") +
  ggtitle("Average Citations with 95% Confidence Interval and Title Length")
```
