...
- References data should be more correct. Since we found that the references given in this dataset are obviously incorrect for several papers. The should be another data sources or method to create better lists of references. For example, we can go through the shrunk of pdf files and generate them ourselves, but that would be too much computation for the data cleaning process.
- If we have data of the time (year) that a paper is cited, we would be able to find some interesting patterns and answer questions such as "Within the same time interval, how much impact does a paper create?"
